...
class UnitsPlugin:
    ...
    def cmd_convert(self, issuedBy, data):
        """[quantity] [unit] - convert unit a to unit b"""
        if not data:
            return
        ...
        if match is None:
            self.bot.reply("\x02{}\x02 is not a valid quantity".format(fromQ))
            return
        ...
        if u is None:
            self.bot.reply("\x02{}\x02 is not a valid unit".format(unit))
            return

        u2 = self.strToUnit(toU)
        if u2 is None:
            self.bot.reply("\x02{}\x02 is not a valid unit".format(toU))
